class ProjectDataChild < ApplicationRecord
  ...
  def self.show_data_new project_type_id, date_last_row, time_last_row, page
    ...
    value.each do |row|
      ...
      data.push("id":row.id, "project_data_id": row.project_data_id, "project_field_id": row.project_field_id,  "form_values":form, "updated_at":row.updated_at,  "user_id": row.user_id )
      @data = data
    end
  end
