class ProjectDataChild < ApplicationRecord
  ...
  def self.row_quantity_children project_type_id, updated_sequence, row_active, current_user
    @rows = ProjectDataChild.joins(:project).row_active(row_active).where("projects.project_type_id = ?", project_type_id).where('project_data_children.update_sequence > ?', updated_sequence).where.not('project_data_children.user_id = 74').select("project_data_children.update_sequence")
    @owner = ProjectFilter.where(user_id: current_user).where(project_type_id: project_type_id).pluck(:owner).first
